...
def port_warning(PCAPS, host_ip):
    with open('utils/protocol/WARN', 'r', encoding='UTF-8') as f:
        warns = f.readlines()
    WARN_DICT = dict()
    for warn in warns:
        warn = warn.strip()
...
def web_warning(PCAPS, host_ip):
    with open('utils/warning/HTTP_ATTACK', 'r', encoding='UTF-8') as f:
        attacks = f.readlines()
    ATTACK_DICT = dict()
    ...
    for ip, count in warn_ip.items():
        webwarn_list.append({'ip_port': ip, 'warn':u'HTTP', 'time': str(count), 'data':None})
    return webwarn_list
